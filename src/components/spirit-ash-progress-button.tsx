...
export function SpiritAshProgressButton({ id, isCollected, name }: { readonly id: string; readonly isCollected: boolean; readonly name: string }) {
  ...
  return <Pressable accessibilityRole="checkbox" accessibilityLabel={`${isCollected ? 'Mark' : 'Collect'} ${name}`} accessibilityState={{ checked: isCollected }} onPress={() => { void toggleSpiritAshCollected(id); }} style={({ pressed }) => [styles.button, { backgroundColor: isCollected ? theme.colors.success : theme.colors.primary, borderRadius: theme.borderRadius.medium, opacity: pressed ? 0.7 : 1 }]}>
  <Text style={{ color: isCollected ? theme.colors.primaryContrast : theme.colors.primaryContrast, fontWeight: '700' }}>{isCollected ? 'Collected' : 'Mark collected'}</Text>
  </Pressable>;
}
